...
var Drag={
	...
	"drag":function(e){
		...
		//设置拖动边界
		var objWidth=handle.root.offsetWidth;
		var objHeight=handle.root.offsetHeight;
		var availHeight=document.documentElement.clientHeight-objHeight;
		var availWidth=document.documentElement.clientWidth-objWidth;
		currentLeft=currentLeft>0?currentLeft:0;
		currentTop=currentTop>0?currentTop:0;
		currentLeft=currentLeft>availWidth?availWidth:currentLeft;
		currentTop=currentTop>availHeight?availHeight:currentTop;
...
Dialog.prototype.setPosition = function () {
    ...
    var dyTop=Math.round(thisTop)-100;
    var dyLeft=Math.round(thisLeft-this.BorderRadius);
    //防止边界溢出
    thisdialogDiv.style.top = (dyTop>10?dyTop:10) + "px";
   var AH=bd.clientHeight-thisdialogDiv.scrollHeight-6;
    if((dyTop+200)>AH){
    	thisdialogDiv.style.top = AH + "px";
    }
    thisdialogDiv.style.left = (dyLeft>0?dyLeft:0) + "px";
};
...
Dialog.prototype.create = function () {
    ...
    var html = '\
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: none; font-size:12px; font-family="inherit"  line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
     \
      <td height="44" style="background-color:' + this.BackGround + ';"><div style="float: left; height:20px;line-height:20px; font-size:15px;  color:#000;"><span id="_Title_' + this.ID + '">' + this.Title + '</span></div>\
        <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);"   style=" position: relative; cursor: pointer; float: right; height: 20px; width: 28px; color:#000; ' + (this.ShowCloseButton ? "" : "display:none;") + '"><span class="glyphicon glyphicon-remove" style="font-size:15px;"></span></div></td>\
      \
    </tr>\
    <tr valign="top">\
      \
      <td align="center"><table width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#ffffff !important">\
          <tr id="_MessageRow_' + this.ID + '" style="' + (this.ShowMessageRow ? "" : "display:none") + '">\
            <td valign="top" height="50"><table width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#eaece9" id="_MessageTable_' + this.ID + '">\
                <tr>\
                  \
                  <td align="left" style="line-height: 15px;"><div id="_MessageTitle_' + this.ID + '" style="font-weight:bold">' + this.MessageTitle + '</div>\
...
'
    ...
    div.style.border="6px solid "+this.BackGround;
    div.style.borderRadius=this.BorderRadius+"px";
    div.style.boxShadow="0px 3px 7px  #000000";
    div.innerHTML = html;
    ...
    if (this.Modal) {
		...
		if(bgdiv.style.display == "none"){
			if(this.Animator){
        		...
        		bgdiv.style.display = "";
				fadeEffect(bgMask,0,50,isIE6?20:10);
				bgMask=null;
			}else{
        		bgdiv.style.display = "";
			}
...
Dialog.getBgdiv = function () {
    ...
    if (!bgdiv) {
		...
		var bgIframeMask = '<div id="_DialogBGMask" style="position:absolute;background-color:#000;opacity:0.5;filter:alpha(opacity=50);width:100%;height:100%;"></div>';
