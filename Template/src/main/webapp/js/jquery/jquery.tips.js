...
function isNull(vlidArray,msgArray,tip) {
	for(var i=0;i<vlidArray.length;i++){
		if(vlidArray[i]==null||vlidArray[i]==""){
			tip.tips({msg : msgArray[i]});
			return false;
		}
	}	
	return true;
}
function checkLen(vlidArray,lenRule,length,msgArray,tip) {
	for(var i=0;i<=vlidArray.length;i++){
		if(lenRule[i]=="gt&lt"){
			if(vlidArray[i].length<length[i][0]||vlidArray[i].length>length[i][1]){
				tip.tips({msg : msgArray[i]});
				return false;
			}
		}else if(lenRule[i]=="gt"){
			if(vlidArray[i].length<length[i][0]){
				tip.tips({msg : msgArray[i]});
				return false;
			}
		}else if(lenRule[i]=="lt"){
			if(vlidArray[i].length>length[i][0]){
				tip.tips({msg : msgArray[i]});
				return false;
			}
		}else if(lenRule[i]=="eq"){
			if(vlidArray[i].length!=length[i][0]){
				tip.tips({msg : msgArray[i]});
				return false;
			}
